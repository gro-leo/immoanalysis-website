...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateCalculations() {
    ...
    const monatsmiete = parseFloat(mieteInput.value);
    const ekProzent = parseFloat(ekInput.value);

    // Update Slider Value Badges
    kaufpreisVal.textContent = euroFormat.format(kaufpreis);
    mieteVal.textContent = euroFormat.format(monatsmiete) + ' / Mo.';
    ekVal.textContent = `${ekProzent} % (${euroFormat.format(kaufpreis * (ekProzent / 100))})`;
    ...
    // Render KPIs
    bruttoRenditeEl.textContent = percentFormat.format(bruttoRendite);
    nettoRenditeEl.textContent = percentFormat.format(nettoRendite);
    ...
    ekRenditeEl.textContent = percentFormat.format(ekRendite);
